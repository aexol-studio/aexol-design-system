...
export const WorkshopTextTile: React.FunctionComponent<
  IWorkshopTextTileProps
> = props => {
  const { text, corner = 'topLeft', style, ...restProps } = props
  ...
  return (
    <div
      ...
      {...restProps}
    >
      <PMedium style={{ color: Colors['Black Hole'] }}>{text}</PMedium>
    </div>
  )
}
